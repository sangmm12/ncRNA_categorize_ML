...
# 按占比从大到小排序标签
sorted_labels = label_percentages.sort_values(ascending=False).index.tolist()
...
# 隐藏多余的子图（如果标签不足18个）
#for i in range(len(sorted_labels), 18):
for i in range(len(sorted_labels), 15):
...
#plt.savefig('probability_distribution_by_label_ncRNA.png', dpi=300, bbox_inches='tight')
#plt.savefig('probability_distribution_by_label_ncRNA.pdf', dpi=300, bbox_inches='tight')
...
# 打印每个标签的详细统计信息（按占比排序）
print("\nDetailed statistics by label (sorted by percentage of total):")
stats_summary = []
